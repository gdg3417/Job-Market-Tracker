...
def test_capacity_workflow_exists_and_runs_scheduled_audits():
    ...
    assert "python -m src.workbook_capacity_hotfix --audit --enforce-critical" in text
...
def test_compaction_requires_explicit_manual_apply_and_formatting_approval_inputs():
    ...
    assert "python -m src.workbook_capacity_hotfix --compact --apply" in text
    assert "github.event_name != 'workflow_dispatch' || !inputs.apply_compaction" in text
...
def test_capacity_workflow_runs_focused_tests_and_schema_validation():
    ...
    assert "tests/test_workbook_capacity.py" in text
    assert "tests/test_workbook_capacity_hotfix.py" in text
    assert "tests/test_workbook_capacity_workflow.py" in text
