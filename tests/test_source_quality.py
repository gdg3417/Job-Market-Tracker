...
def test_probe_classifies_healthy_page():
    session = FakeSession(FakeResponse(text='<script type="application/ld+json">{"@type":"JobPosting"}</script>'))

    result = probe_source("https://example.com/careers", session=session, observed_at="2026-07-14T12:00:00Z")
...
def test_probe_classifies_empty_but_valid_page():
    ...
    result = probe_source("https://example.com/careers", session=session)
...
def test_probe_classifies_redirect_replacement():
    ...
    result = probe_source("https://example.com/old-careers", session=session)
...
def test_one_404_does_not_disable_or_require_configuration_change():
    probe = SourceProbe(
    ...
    decision = retry_decision(probe, failure_observations=1, as_of=datetime(2026, 7, 14, tzinfo=UTC))
...
def test_repeated_404_requires_config_change_before_retry():
    probe = SourceProbe(
        source_url="https://example.com/jobs",
        final_url="https://example.com/jobs",
        classification=PERMANENT_404,
    ...
    decision = retry_decision(probe, failure_observations=2, as_of=datetime(2026, 7, 14, tzinfo=UTC))
...
def test_dns_failures_use_cooldown_and_bounded_retry():
    ...
    probe = probe_source("https://missing.example/jobs", session=session)

    first = retry_decision(probe, failure_observations=1, as_of=datetime(2026, 7, 14, tzinfo=UTC))
    third = retry_decision(probe, failure_observations=3, as_of=datetime(2026, 7, 14, tzinfo=UTC))
...
def test_403_is_temporary_and_never_permanent_from_one_observation():
    session = FakeSession(FakeResponse(status_code=403, text="Forbidden"))

    probe = probe_source("https://example.com/jobs", session=session)
    decision = retry_decision(probe, failure_observations=1, as_of=datetime(2026, 7, 14, tzinfo=UTC))
...
def test_temporary_server_failure_recovers_with_bounded_cooldown():
    session = FakeSession(FakeResponse(status_code=503, text="Unavailable"))

    probe = probe_source("https://example.com/jobs", session=session)
    decision = retry_decision(probe, failure_observations=3, as_of=datetime(2026, 7, 14, tzinfo=UTC))
...
def test_audit_uses_run_history_to_confirm_repeated_404():
    ...
    session = FakeSession(FakeResponse(status_code=404, text="Not found", url="https://retired.example/jobs"))
...
def test_approved_redirect_update_preserves_history_and_requires_exact_company_id():
    row = {
        "company_id": "example",
    ...
    no_updates = apply_approved_source_updates(
        [(2, row)],
        [finding],
...
def test_repeated_404_can_be_retired_only_after_explicit_approval():
    row = {
        "company_id": "retired",
        "company_name": "Retired Co",
        "source_url": "https://retired.example/jobs",
...
def test_four_week_yield_calculations_and_recommendations():
    ...
    gmail_row = next(row for row in rows if row.group_type == "gmail_alert_or_search" and row.group_key == "Dallas strategy jobs")
...
def test_strategic_zero_result_source_is_kept():
    rows = build_source_yield_report(
        jobs=[],
        job_sources=[],
        rejected_jobs=[],
...
def test_low_volume_source_is_not_automatically_retired():
    jobs = [
        {
            "job_key": "job1",
